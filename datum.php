...
$pathname ="";
$id ="";
	$verbindung = mysql_connect("localhost", "counter" , "W9g9jWVPFGZWC2TspdWp") or die("Verbindung zur Datenbank konnte nicht hergestellt werden"); 
	mysql_select_db("counter") or die ("Datenbank konnte nicht ausgewählt werden"); 
		$day_week = date("D");
		$day_next = date("d"); 
		$month = date("m");
		$year = date("Y");
...
if( isset ($_GET['datum']) and !isset ($_POST['datum'])  ) {$datum = $_GET['datum']; 
   ...
   while($row_tagn = mysql_fetch_row($result_tagn))
	$dayname = $row_tagn[0];
if( $dayname =="Mo" ) {$datum_anz="Montag, ";}
if( $dayname =="Di" ) {$datum_anz="Dienstag, ";}
if( $dayname =="Mi" ) {$datum_anz="Mittwoch, ";}
if( $dayname =="Do" ) {$datum_anz="Donnerstag, ";}
if( $dayname =="Fr" ) {$datum_anz="Freitag, ";}
